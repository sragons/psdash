...
class PsDashRunner:
    ...
    def _setup_logging(self):
        level = (
            self.app.config.get("PSDASH_LOG_LEVEL", logging.INFO)
    ...
    def _setup_workers(self):
        ...
        if self.app.config.get("PSDASH_AGENT"):
            ...
            gevent.spawn_later(
                register_interval, self._register_agent_worker, register_interval
            )
    ...
    def _run_web(self):
        ...
        self.server = WSGIServer(listen_to, application=self.app, log=log, **ssl_args)
